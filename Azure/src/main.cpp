...
int main()
{
    ...
    matProj = glm::perspective(glm::radians(60.0f), width / (float) height, 0.1f, 50.0f);
    ...
    planet->init("models/planet.obj", blinnShader);

    std::shared_ptr<Mesh> rocks = std::make_shared<Mesh>();
    rocks->init("models/rocks.obj", texblinnShader);
    ...
    std::shared_ptr<Node> rocksNode = std::make_shared<Node>();
